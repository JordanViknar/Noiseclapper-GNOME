...
export abstract class SoundcoreClient {
    ...
    public abstract setSettingsValue(mac: string, name: string, value: string): Promise<void>;
...
export class OpenSCQ30Client extends SoundcoreClient {
    ...
    public async getDevices(): Promise<Device[]> {
        ...
        return stdout
            ...
            .map(([model, mac]) => ({ model: OpenSCQ30Client.POSSIBLE_MODEL_NAMES[model.slice("Soundcore".length)], mac }))
    }

    // All of this just to parse a table
    public async getSettingsValue(mac: string, name: string): Promise<string> {
        const stdout = await this.runOpenSCQ30(["device", "exec", "--get", name, "--mac-address", mac])
    ...
    public async setSettingsValue(mac: string, name: string, value: string): Promise<void> {
        await this.runOpenSCQ30(["device", "exec", "--set", `${name}=${value}`, "--mac-address", mac])
    }

    public async getAvailableSettings(mac: string): Promise<string[]> {
        const stdout = await this.runOpenSCQ30(["device", "list-settings", "--no-headers", "--no-extended-info", "--mac-address", mac])
    ...
    public async addNewDevice(mac: string, model: string): Promise<void> {
        const entry = Object
            .entries(OpenSCQ30Client.POSSIBLE_MODEL_NAMES)
            .find(([serial, m]) => m.toLowerCase() === model.toLowerCase())
        ...
        await this.runOpenSCQ30(["paired-devices", "add", "--mac-address", mac, "--model", openscq30ModelName])
    }
...
export class PythonClient extends SoundcoreClient {
    ...
    public async isModelSupported(model: string): Promise<boolean> {
        ...
        return PythonClient.SUPPORTED_MODELS.map(model => model.toLowerCase()).includes(model.toLowerCase())
    }
    ...
    private getSignal(settings: string, value: string) {
        if (settings === "noiseCancellingMode") {
            ...
            return PythonClient.SIGNAL_NOISE_CANCELLING_COMMAND + PythonClient.SIGNAL_NOISE_CANCELLING_VALUES[value]
        }
        ...
        if (settings === "presetEqualizerProfile") {
            ...
            return PythonClient.SIGNAL_EQUALIZER_PRESET_COMMAND + PythonClient.SIGNAL_EQUALIZER_PRESENT_VALUES[value]
        }
